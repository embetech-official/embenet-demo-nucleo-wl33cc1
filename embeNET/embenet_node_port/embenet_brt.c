...
static uint16_t openhdlc_crc(uint16_t crc, uint8_t byte) { return (crc >> 8) ^ fcstab[(crc ^ byte) & 0xff]; }

// ring buffer descriptors
static RingBuffer outputRingBufferDesc;
static RingBuffer inputRingBufferDesc;

// input and output buffers
static uint8_t outputRingBuffer[OUTPUT_RING_BUFFER_SIZE];
static uint8_t inputRingBuffer[INPUT_RING_BUFFER_SIZE];

// true if transmission is in progress, otherwise false
static bool volatile isTransmitting;
 ...
 */
static void tx_isr(void *context);
